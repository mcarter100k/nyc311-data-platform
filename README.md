...
Two populations, deliberately not summed: **<!--claim:test_count-->131<!--/claim--> pytest tests** that need no cloud account, and **119 dbt data tests (113 generic + 6 singular)** that run against the warehouse during `dbt build`. The pytest count is recomputed in CI.

| Tier | Count | What it proves |
|---|---|---|
| **Structural** | 90 | Configuration correctness — schema resolution, incremental strategy, DAG lineage, freshness target, Terraform validity, the LOADER-has-no-TRUNCATE contract, a relationships test on every fact foreign key |
