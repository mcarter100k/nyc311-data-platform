...
<a name="test-suite"></a>
## Test Suite — <!--claim:test_count-->89<!--/claim--> Tests, Zero Live Credentials Required

The suite has two tiers, neither of which connects to Snowflake, Databricks, or Azure (the total above is recomputed by `scripts/check_claims.py` in CI — the build fails if this section drifts):
