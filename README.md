...
Two populations, deliberately not summed: **<!--claim:test_count-->117<!--/claim--> pytest tests** that need no cloud account, and **110 dbt data tests** that run against the warehouse during `dbt build`. The pytest count is recomputed in CI.
...
tests/          117 pytest tests across three tiers
```
